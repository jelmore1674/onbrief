...
export async function getJobData(tokens: ApiTokens) {
    try {
        ...
        const jobs = Content.reduce((acc, job) => {
            const legs = job.Cargos.reduce((cargoAcc, cargo) => {
                const pax = job.Charters.find(
                    (pax) =>
                        pax.DepartureAirport.ICAO === cargo.DepartureAirport.ICAO &&
                        pax.DestinationAirport.ICAO === cargo.DestinationAirport.ICAO,
                );
        ...
        const levelJobs = companyJobs.filter((job) => job.expires === undefined);
        const pendingJobs = companyJobs.filter((job) => job.expires !== undefined);
